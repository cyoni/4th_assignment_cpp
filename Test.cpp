...
int main() {
	...
	if (signal == 0) {
		...
		.CHECK_OUTPUT(calculateBullAndPgia("55555","55555"), "5,0") 
		...
		.CHECK_EQUAL(play(c12, g12, 2, 5), 1)
		...
		for (uint i=0; i<100; ++i) {
        	...
        	for (uint i=0; i<100; ++i) {
		testcase.CHECK_EQUAL(play(randy2, smarty2, 5, 100)<=50, true);
		}
		
		for (uint i=0; i<100; ++i) {
		testcase.CHECK_EQUAL(play(randy2, smarty2, 2, 100)<=20, true);
		}
		

		for (uint i=0; i<100; ++i) {
		testcase.CHECK_EQUAL(play(randy2, smarty2, 4, 100)<=40, true);
		}
