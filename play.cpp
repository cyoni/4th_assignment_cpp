...
namespace bullpgia {
	uint play(Chooser& chooser, Guesser& guesser, uint length, uint maxTurns) {
		...
		uint indexOfTurn;
		for (indexOfTurn=0; indexOfTurn<maxTurns; ++indexOfTurn) { // loop that runs as long as you have turns
			string guess = guesser.guess(); // take a guess
			if (guess.length()!=length){  // Illegal guess
				cout << "illegal guess size is diff " << guess.length() << "," << length <<" GUESS=" << guess << endl;
				return TECHNICAL_VICTORY_TO_CHOOSER;
				}
			if (guess==choice) {
			cout << "won" << endl;
				return indexOfTurn + 1; // returns - number of turn+1
			} else {
